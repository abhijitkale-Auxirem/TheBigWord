...
const Press: React.FC = () => {
  const handleDownloadRelease = (title: string) => {
    toast.success(`Starting download: "${title}" Press Release (PDF)`);
  };

  const handleDownloadMediaKit = () => {
    toast.success('Starting download: TheBigWord Media Kit (ZIP)');
  };
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
            ...
            <div key={pr.title} className="bg-white rounded-2xl border border-border p-6 hover:shadow-lg transition-shadow group">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  <button 
                    onClick={() => handleDownloadRelease(pr.title)}
        ...
        <div className="bg-brand-surface rounded-3xl border border-border p-8 text-center animate-fade-in">
          ...
          <button 
            ...
            className="flex items-center gap-2 gradient-primary text-white font-semibold px-6 py-3 rounded-xl hover:opacity-95 transition-opacity shadow-lg mx-auto"
          >
